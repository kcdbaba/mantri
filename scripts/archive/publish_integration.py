...
def _run_history(runs: list[dict], cases: list[dict]) -> str:
    ...
    if live:
        ...
        def _col_has_data(key):
            if key.startswith("_"):
                return True  # always show composite columns
            return any(r.get(key) for r in live)

        def _cell(r, key, case_data=None):
            if key == "_mode":
            ...
            if key == "_models":
                ...
                if case_data:
                    return _fmt_model_usage(case_data.get("_model_usage", []))
                return "\u2014"
            if key == "_cost":
        ...
        for case_id in sorted(live_by_case.keys()):
            ...
            full_runs = [r for r in case_runs if not r.get("skip_linkage") and not r.get("max_messages")]
            latest = full_runs[0] if full_runs else case_runs[0]
            ...
            for r in case_runs:
                ...
                if notes:
                    n_cols = len(active_cols) + 1
                    escaped_notes = html_mod.escape(notes, quote=True)
                    rows.append(
                        f"<tr class='group-child' data-group='{group_key}'>"
                        f"<td colspan='{n_cols}' class='run-note'>"
